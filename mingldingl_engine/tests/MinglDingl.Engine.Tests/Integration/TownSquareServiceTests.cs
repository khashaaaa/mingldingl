...
public class TownSquareServiceIntegrationTests : IntegrationTestBase
{
    ...
    // A session is created Open, so the RSVP-opens date the admin sets is the only thing standing
    // between a scheduled gathering and a roster that fills days early.
    [Fact]
    public async Task RsvpAsync_BeforeRsvpOpens_Throws()
    {
        var user = NewCompleteUser();
        var session = NewSession();
        session.RsvpOpensAt = DateTime.UtcNow.AddDays(1);
        session.RsvpClosesAt = DateTime.UtcNow.AddDays(2);
        session.ScheduledStartAt = DateTime.UtcNow.AddDays(2).AddHours(1);
        Db.Users.Add(user);
        Db.TownSquareSessions.Add(session);
        await Db.SaveChangesAsync();
        ...
        Assert.Equal("square.rsvp_not_open", ex.Code);
        Db.ChangeTracker.Clear();
