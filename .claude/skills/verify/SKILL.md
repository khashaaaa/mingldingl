...
- **Login trick:** `useAuth` no longer has a dev stub — phone verification is real (verify.mn Mobile-Originated SMS; see CLAUDE.md). The web login flow is now: enter 8 digits → the app calls `POST /auth/phone/start` and shows verify.mn's instruction + an `sms:` button → it polls `GET /auth/phone/status/{id}` until VERIFIED → then Supabase sign-up + `POST /auth/phone/claim`. For Playwright you still route-intercept `**/auth/v1/signup**` with a saved real session JSON, but you must ALSO make the engine consider the phone verified, otherwise `POST /users` returns 403 and the user is stranded in onboarding. Easiest: insert a `PhoneVerifications` row with `Status = 1` (Verified) and `ClaimedByUserId` = the Supabase `sub`, then the app proceeds. There is no "any 6 digits" shortcut any more.
...
- **Raw-SQL seeding gotchas** (writing INSERT statements directly against local Postgres, e.g. for bulk test data):
  - **`mingldingl_engine/scripts/reseed-dev-db.sql` wipes and reseeds the whole dev database** (reference content, 40 varied users, businesses, matches/messages, ships, Town Square, lifecycle states). Start there rather than hand-writing INSERTs. It deliberately does NOT truncate `AdminConfigs` (the seeder owns it) or `ContentPages` (authored product copy — the guides body is real content, not a fixture).
  - **Run `python3 mingldingl_engine/scripts/gen-seed-photos.py` after every reseed.** The seed writes `PhotoUrls` pointing at `/uploads/photos/seed/…`, but `uploads/` is gitignored, so without this every seeded profile, candidate card and business photo 404s and renders as a blank placeholder. The script reads the URLs the DB actually holds and fills in whatever is missing (needs Pillow); `--force` rewrites existing files.
  - `User.PhotoUrls` and `BusinessPartner.PhotoUrls` are `jsonb`, not a Postgres array — use `'["url1","url2"]'::jsonb`, not `ARRAY[...]::text[]`. But `QuizQuestions.Options` **is** `text[]` while `Icebreakers.Options` is `jsonb` — same concept, two storage types.
  - The `messages` table is the one on snake_case columns (`id`, `match_id`, `sender_id`, `content`, `created_at`) and a lowercase name, via an explicit `ToTable("messages")`. Everything else is PascalCase.
  - Tier ladder is Garnet 0 / Opal 100 / Amethyst 300 / Sapphire 600 / Ruby 1000 / Emerald 2000 (`ScoreService.TierDefaults`; every threshold above Garnet is config-tunable via `tier.<name>.threshold`, and the seeder resets the whole ladder to defaults if the stored values are out of order). There is no Diamond tier — seeding a `GemTier` that disagrees with `TotalScore` makes behaviour testing meaningless.
