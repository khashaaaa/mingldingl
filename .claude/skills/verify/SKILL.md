...
- **`POST /dev/run-maintenance-sweep`** (Development-only, 404 elsewhere) triggers the hourly `DailyMaintenanceBackgroundService` sweep (ghosting, daily-reset, deletion-anonymization, membership-expiry) on demand — use this instead of waiting up to an hour to verify sweep-driven behavior.
