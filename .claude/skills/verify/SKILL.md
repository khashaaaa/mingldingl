...
- **Raw-SQL seeding gotchas** (writing INSERT statements directly against local Postgres, e.g. for bulk test data):
  ...
  - Tier ladder is Garnet 0 / Opal 100 / Amethyst 300 / Sapphire 600 / Ruby 1000 / Emerald 2000 (`ScoreService.TierDefaults`; Sapphire's is config-tunable). There is no Diamond tier — seeding a `GemTier` that disagrees with `TotalScore` makes behaviour testing meaningless.
