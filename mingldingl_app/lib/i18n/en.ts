...
export const en = {
  ...
  round_over_matches: 'Lanterns lit from both sides:',
  round_over_no_matches: 'No lantern lit from both sides this time. The next gathering will be along.',
  // The plaza board (Task 7, move 9): the count-aware line above the lit rows. Two keys rather
  // than one templated sentence, the same way `keepsake_line`/`_one` split zero/one/many by hand.
  plaza_closed_lit: '%{count} lanterns were lit from both sides. They wait in your letters.',
  plaza_closed_lit_one: 'One lantern was lit from both sides. It waits in your letters.',
  ...
  rite_explainer: '%{minutes} minutes by the fire before you meet. Ask, and they answer.',
  rite_propose_cta: 'Ask',
  rite_waiting: 'You have asked. A candle until they answer.',
  rite_incoming: 'They have asked for it.',
  rite_accept_cta: 'Accept',
  rite_decline_cta: 'Not yet',
  rite_ready: 'The rite is open. Step in when you are both ready.',
  rite_complete: 'Flame-tested',
  // New in Task 11 (Wave 3): the complete state's second line, under the flame glyph.
  rite_complete_sub: 'Two faces met across the glass. The seal on the likeness is gone for good.',
  ...
  town_square_rsvp: 'Light it',
  town_square_cancel_rsvp: 'Put out',
  ...
  town_square_rejoin: 'Return',
  // The Square as a plaza (Task 7, move 9): the hero card's sub line, its two stat rows, and the
  // drawing's own accessible label. `plaza_sub_*` all take the same `%{count}` — the roster's own
  // `rsvpCount` — so the three read as one sentence with the ending swapped for who is standing in
  // the square. `plaza_label*` mirror that same split for the plaza's own accessibility label.
  plaza_sub_open: 'Gates close at the lantern-lighting. %{count} lanterns lit so far, yours among them.',
  plaza_sub_open_not_mine: 'Gates close at the lantern-lighting. %{count} lanterns lit so far.',
  plaza_sub_locked: 'The gates are shut. %{count} lanterns lit. The first bell is near.',
  plaza_first_bell: 'First bell',
  plaza_rounds: 'Rounds',
  plaza_rounds_value: '%{count}, a bell each',
  ...
  video_end_confirm_title: 'Douse the fire?',
  video_end_confirm_body: 'Dousing ends the rite for both of you. It cannot be relit.',
  video_end_confirm: 'Douse it',
  video_connect_error_title: 'The way would not open.',
  video_connect_error_body: 'We could not connect the call. Try again, or go back to the thread; nothing was lost.',
  // New in Task 11 (Wave 3): the video screen's own retry label. `rejoin` stays as the Square
  // round's "Rejoin" — the two screens no longer share a button word.
  rite_try_again: 'Try again',
  gem_garnet: 'Garnet',
  gem_opal: 'Opal',
