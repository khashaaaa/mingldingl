...
describe('StateBlock — waiting, empty, wrong', () => {
  ...
  it('lets the tone pick the family, not the icon name', () => {
    const { getByTestId, queryByTestId } = render(
      <StateBlock tone="danger" icon="wifi-off" title="Nothing came back" />,
    );

    expect(getByTestId('state-ember', HIDDEN)).toBeTruthy();
    expect(queryByTestId('state-place-window-dark', HIDDEN)).toBeNull();
  });
  ...
  ] as const)('draws %s as the place it stood for', (icon, place) => {
    const { getByTestId, queryByTestId } = render(<StateBlock icon={icon} title="Quiet here" />);
  ...
  // 27 call sites pass a stock name and only six of them have a drawing. The other twenty-one
  // must go on rendering exactly what they rendered yesterday.
  it('falls back to the stock icon for a name with no drawing', async () => {
    const { findByText, queryByTestId } = render(
      <StateBlock icon="party-popper" title="The square stands quiet" />,
    );
