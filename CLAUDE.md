...
- **Realtime chat/nudges use Supabase's Broadcast API, not `postgres_changes`.** `postgres_changes` only observes Supabase's own hosted Postgres, so it silently stopped working once data moved to local Postgres. The engine (`SupabaseBroadcastService`) explicitly pushes an event after each relevant write. Topics: `chat:{matchId}` (message INSERTs), `townsquare:{sessionId}` (session/round lifecycle), and **`user:{Users.Id}`** for everything about a match — message/icebreaker/quiz/date_confirmed nudges, flame_rite_*, match_created, match_status_changed — sent via `BroadcastToUsersAsync` to *both participants'* topics, never a shared one (a single `app-nudges` topic once delivered every user's events to every client). The app's `useRealtimeNudges` subscribes to `user:{engine user id}` (the profile id, never the Supabase `sub`) and still drops events for matches it has not cached. These are **public** channels — the topic name is not authorisation; private-channel RLS is not set up yet. Every app subscription goes through `lib/realtime/subscribeWithRetry.ts` (`subscribeToBroadcast`): one ref-counted channel per topic with handler fan-out, retry with backoff on CHANNEL_ERROR/TIMED_OUT/CLOSED, and no re-creation until the previous `removeChannel` settles — never call `supabase.channel()` directly, because it hands back a still-leaving channel whose `subscribe()` silently no-ops. If a live-update feature seems to "just not fire," check the engine log for the broadcast POST before suspecting the client.
...
- **`EXPO_PUBLIC_API_URL` is still unset for `preview` and `production`.** Only the `development` profile pins it (the LAN IP, for on-device testing against the local engine). `lib/api/api.ts` falls back to `http://localhost:5150`, which on a phone means the phone itself — so a preview or production build made today would silently talk to nothing. Set it on the profile, or as an EAS variable in that environment, before the first non-development build. A development build is unaffected: its JS is served by local Metro, so it reads the local `.env` at bundle time.
